  ...
  "fixed z-50 flex flex-col gap-4 gap-4 border-default-700 bg-default-950 p-6 shadow-lg transition ease-in-out data-[state=closed]:animate-out data-[state=open]:animate-in data-[state=closed]:duration-300 data-[state=open]:duration-500",
  {
    variants: {
      side: {
        ...
        left: "data-[state=closed]:slide-out-to-left data-[state=open]:slide-in-from-left inset-y-0 left-0 h-full w-3/4 w-full min-w-80 max-w-sm border-r",
        right:
          "data-[state=closed]:slide-out-to-right data-[state=open]:slide-in-from-right inset-y-0 right-0 h-full w-3/4 w-full min-w-80 max-w-sm border-l",
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Title ref={ref} className={cn("font-extrabold text-xl", className)} {...props} />
));
